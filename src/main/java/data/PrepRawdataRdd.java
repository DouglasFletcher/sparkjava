...
public class PrepRawdataRdd {
    ...
    public static JavaRDD<LabeledPoint> createRandForestData(JavaRDD<String[]> datasetIn) {
        ...
        return datasetIn.map(s -> {
            ...
            switch (s[11]) {//
                case "0": //
                    NumberOfDependents=0; break; //
                case "NA": //
                    NumberOfDependents=0; break; //
                default: //
                    NumberOfDependents=Integer.parseInt(s[11]); break; //
            }
            ...
            if(UnknownMonthlyIncome == 1){//
                MonthlyIncome = 0;//
            } else{//
                MonthlyIncome = Double.parseDouble(s[6]);//
            }
            ...
            if(UnknownMonthlyIncome == 0){//
                UnknownIncomeDebtRatio = (double) 0;//
            } else{//
                UnknownIncomeDebtRatio = Double.parseDouble(s[5]);//
            }
            ...
            if(UnknownMonthlyIncome == 1){//
                ...
                DebtRatio = Double.parseDouble(s[5]);//
            }

            double WeirdRevolvingUtilization = (double) 0;
            if (Math.log(Double.parseDouble(s[2])) > 3){//
                WeirdRevolvingUtilization = Double.parseDouble(s[2]);//
            }

            int ZeroRevolvingUtilization=0;
            if (s[2].equals("0")){//
                ZeroRevolvingUtilization=1;//
            }

            double RevolvingUtilizationOfUnsecuredLines = (double) 0;
            if (Math.log(Double.parseDouble(s[2])) <= 3){//
                RevolvingUtilizationOfUnsecuredLines = Double.parseDouble(s[2]);//
            }

            double LogDebt;
            if(MonthlyIncome <= 1){//
                LogDebt=Math.log(DebtRatio);//
            } else{ //
                LogDebt=Math.log(MonthlyIncome * DebtRatio);//
            }
            ...
            if(Integer.parseInt(s[9]) > 0) {//
                HasRealEstateLoans = 1;//
            }

            int HasMultipleRealEstateLoans = 0;
            if(Integer.parseInt(s[9]) > 2){//
                HasMultipleRealEstateLoans = 1;//
            }
            ...
            int DTIOver33 = 0;
            if(NoIncome == 0 && DebtRatio > 0.33){//
                DTIOver33 = 1;//
            }

            int DTIOver43 = 0;
            if(NoIncome == 0 && DebtRatio > 0.43){//
                EligibleSS = 1;//
            }

            double DisposableIncome = (double) 0;
            if(NoIncome!=1){//
            ...
            if(Double.parseDouble(s[4]) > 90){//
                NumberOfTime3059DaysPastDueNotWorseLarge = 1;//
            }

            int NumberOfTime3059DaysPastDueNotWorse96 = 0;
            if(Double.parseDouble(s[4]) == 96){//
                NumberOfTime3059DaysPastDueNotWorse96 = 1;//
            }

            int NumberOfTime3059DaysPastDueNotWorse98 = 0;
            if(Double.parseDouble(s[4]) == 98){//
                NumberOfTime3059DaysPastDueNotWorse98 = 1;//
            }

            int Never3059DaysPastDueNotWorse = 0;
            if(Double.parseDouble(s[4]) == 0){//
                Never3059DaysPastDueNotWorse = 1;//
            }

            int NumberOfTime3059DaysPastDueNotWorse = 0;
            if(Double.parseDouble(s[4]) <= 90){//
                NumberOfTime3059DaysPastDueNotWorse = Integer.parseInt(s[4]);//
            }

            int NumberOfTime6089DaysPastDueNotWorseLarge = 0;
            if(Double.parseDouble(s[10]) > 90){//
                NumberOfTime6089DaysPastDueNotWorseLarge = 1;//
            }

            int NumberOfTime6089DaysPastDueNotWorse96 = 0;
            if(Double.parseDouble(s[10]) == 96){//
                NumberOfTime6089DaysPastDueNotWorse96 = 1;//
            }

            int NumberOfTime6089DaysPastDueNotWorse98 = 0;
            if(Double.parseDouble(s[10]) == 98){//
                NumberOfTime6089DaysPastDueNotWorse98 = 1;//
            }

            int Never6089DaysPastDueNotWorse = 0;
            if(Double.parseDouble(s[10]) == 0){//
                Never6089DaysPastDueNotWorse = 1;//
            }

            int NumberOfTime6089DaysPastDueNotWorse = 0;
            if(Double.parseDouble(s[10]) <= 90){//
                NumberOfTime6089DaysPastDueNotWorse = Integer.parseInt(s[10]);//
            }

            int NumberOfTimes90DaysLateLarge = 0;
            if(Double.parseDouble(s[8]) > 90){//
                NumberOfTime6089DaysPastDueNotWorse = 1;//
            }

            int NumberOfTimes90DaysLate96 = 0;
            if(Double.parseDouble(s[8]) == 96){//
                NumberOfTimes90DaysLate96 = 1;//
            }

            int NumberOfTimes90DaysLate98 = 0;
            if(Double.parseDouble(s[8]) == 98){//
                NumberOfTimes90DaysLate98 = 1;//
            }
            ...
            if(Double.parseDouble(s[8]) <= 90){//
                NumberOfTimes90DaysLate = Integer.parseInt(s[8]);//
            }

            int IncomeDivBy10 = 0;
            if ((MonthlyIncome % 10) == 0){//
                IncomeDivBy10 = 1;//
            }

            int IncomeDivBy100 = 0;
            if ((MonthlyIncome % 100) == 0){//
                IncomeDivBy100 = 1;//
            }
            ...
            if(RevolvingUtilizationOfUnsecuredLines == 0.9999999){//
                Weird0999Utilization = 1;//
            }

            int FullUtilization = 0;
            if(RevolvingUtilizationOfUnsecuredLines == 1){//
                FullUtilization = 1;//
            }
            ...
            int LowAge = 0; if(Integer.parseInt(s[3]) < 18) {//
                LowAge = 1;//
            }
