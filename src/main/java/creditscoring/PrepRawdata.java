...
public class PrepRawdata {
	...
	public static JavaRDD<LabeledPoint> createRandForestData(JavaRDD<String[]> datasetIn){
		...
		// *************************
		JavaRDD<LabeledPoint> datasetOut = datasetIn.map(
			new Function<String[], LabeledPoint>(){
				...
				public LabeledPoint call(String[] s) throws Exception {
					...
					if(s[11].equals("NA")){UnknownNumberOfDependents=1;}
					...
					/*
					double LogUnknownIncomeDebtRatioPerLine;
					if(Double.isNaN(LogDebt-Math.log1p(Integer.getInteger(s[7]))) | 
					Double.isInfinite(LogDebt-Math.log1p(Integer.getInteger(s[7])))){
						LogUnknownIncomeDebtRatioPerLine = 0;
					} 
					else {
						LogUnknownIncomeDebtRatioPerLine = LogDebt-Math.log1p(Integer.getInteger(s[7]));
					}
					
					double LogUnknownIncomeDebtRatioPerRealEstateLine;
					if(Double.isNaN(LogUnknownIncomeDebtRatio-Math.log1p(Integer.getInteger(s[9]))) | 
					Double.isInfinite(LogUnknownIncomeDebtRatio-Math.log1p(Integer.getInteger(s[9])))){
						LogUnknownIncomeDebtRatioPerRealEstateLine=0;
					}
					else {
						LogUnknownIncomeDebtRatioPerRealEstateLine=LogUnknownIncomeDebtRatio-Math.log1p(Integer.getInteger(s[9]));
					}
					*/
